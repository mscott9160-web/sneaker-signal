...
import './App.css'
import { demoReleases, formatLaunchTime, loadReleases, type Release } from './release-data'
...
function releaseStatus(release: Release) {
  const lifecycleStatus = release.status.toLowerCase()
  if (['cancelled', 'sold out', 'postponed'].includes(lifecycleStatus)) return release.status.toUpperCase()
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="top-actions"><span className="region-badge">US ONLY · USD</span><label className="search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search sneakers..." /></label><span className="profile inactive-control" aria-label="Profile unavailable">JD</span></div>
      </header>
